...
#def display_instances(image, boxes, masks, class_ids, filename, threshold, scores=None):
    ...
    for i in range(N):
        ...
        if scores[i] > threshold :  
            ...
            #$$$$$$$$$$
                cimg = np.zeros_like(orig_img)
                cv2.drawContours(cimg, contours, i, color=255, thickness=-1)

                # Access the image pixels and create a 1D numpy array then add to list
                pts = np.where(cimg == 255)
                lst_intensities.append(orig_img[pts[0], pts[1]])
            ...
            for verts in contours:
                ...
                clr = [0,250, 0]
                #print("clr......",clr)
                masked_image = cv2.polylines(masked_image,[verts],True,clr,thickness)

                #x,y,w,h = cv2.boundingRect(verts) # offsets - with this you get 'mask'
                #x= x-10
                #y= y-10              
                #crop_img = orig_img[y:y+h,x:x+w]
                #crop_img = cv2.resize(crop_img,(50,50), interpolation=cv2.INTER_AREA)
                #crop_img = cv2.cvtColor(crop_img, cv2.COLOR_BGR2GRAY)
                             
                #avg_pixel_cell = np.mean(crop_img)
                #mean_val = cv2.mean(orig_img,mask = mask1)
                
                #print("avg_pixel_cell mask1",mean_val)
...
# def display_instances(image, boxes, masks, class_ids, class_names, drtry,
                      # scores=None, title="",
    ...
    # for i in range(N):
        ...
        # if not captions:
            ...
            # clsname = ['alive', 'dead']
            # print(class_id)
            # label = clsname[class_id - 1]     #class_names[class_id]
        ...
        # for verts in contours:
            ...
            # ax.add_patch(p)
    # caption = 'live:' + str(live) + '\ndead:' + str(dead)
    # ax.text(20, 20,caption ,
